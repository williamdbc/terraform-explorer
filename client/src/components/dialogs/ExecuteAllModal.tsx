...
import { X, Loader2, PlayCircle, Command } from "lucide-react";
...
export function ExecuteAllModal({
  ...
  const run = async (cmd: string) => {
    ...
    results.forEach(r => {
      ...
      outputs.set(path, [...existing, { ...r, command: cmd }]);
    });

    setTerminalOutputs(prev => {
      const updated = new Map(prev);
      outputs.forEach((cmds, path) => {
        const current = updated.get(path) || [];
        updated.set(path, [...current, ...cmds]);
      });
      return updated;
    });
    ...
    <>
            ...
            <div className="px-6 py-4 border-b flex items-start gap-3 bg-blue-50 border-blue-200">
              <div className="flex-1 min-w-0">
                <div className="mt-2 space-y-1 text-sm text-blue-700">
                  ...
                  {projectGroupName && <p><span className="font-medium">Grupo:</span> {projectGroupName}</p>}
                  <p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {TERRAFORM_ACTIONS.map((action) => (
                    <Button
                      ...
                      size="sm"
                    >
                      {loading ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <action.icon className="w-3 h-3 mr-1" />}
                      {action.label}
                    </Button>
                  ))}
                </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="ex: plan -var-file=prod.tfvars"
                ...
                <Button
                  ...
                  disabled={!custom.trim() || loading || selectedProjects.length === 0}
                ...
                <Button
                  variant="secondary"
                  className="w-full border border-slate-300 bg-slate-50 hover:bg-slate-100 text-slate-800 font-medium"
      ...
      <BatchTerraformDialog
        ...
        onBatchResults={(results) => {
          ...
          if (results.length > 0 && results[0].workingDir) {
            setActiveTab(results[0].workingDir);
          }
        }}
      />
    </>
